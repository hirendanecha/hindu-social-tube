...
export function app(): express.Express {
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'tube-dist/video-hindu-social/browser'),
        'index.html'
      )
    )
    .toString();
      ...
      async (err, html) => {
        ...
        if (params.indexOf('channel/') > -1) {
          ...
          const talent = {
            ...
            image: post?.thumbfilename || post?.metaimage || post?.imageUrl || 'https://tube.hindu.social/assets/FreedomTube-logo.png',
          };
          seo.title = talent.name;
          seo.description = strip_html_tags(talent.description);
          seo.image = talent.image;
